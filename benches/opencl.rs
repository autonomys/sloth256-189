...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    while layers <= MAX_LAYERS {
        group.bench_function(format!("Encode-parallel/{} GB/{} pieces/{} layer(s)", size, NUM_PIECES.to_formatted_string(&Locale::en), layers), |b| {
            let piece = random_bytes::<{4096 * NUM_PIECES}>();

            b.iter_batched_ref(
                || piece.clone(),
                |mut input| sloth256_189::opencl::encode(&mut input, &expanded_ivs, layers, instances).unwrap(),
                BatchSize::LargeInput,
            );
        });

        group.bench_function(format!("Encode-parallel-pinned/{} GB/{} pieces/{} layer(s)", size, NUM_PIECES.to_formatted_string(&Locale::en), layers), |b| {
            b.iter_custom(|_| {
                let mut piece = sloth256_189::opencl::pinned_memory_alloc(instances, 4096 * NUM_PIECES).unwrap();
                random_bytes_inplace::<{4096 * NUM_PIECES}>(&mut piece);

                let start = Instant::now();
                sloth256_189::opencl::encode(&mut piece, &expanded_ivs, layers, instances).unwrap();
